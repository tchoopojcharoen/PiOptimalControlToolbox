...
%
%   [X,U,T] = DIRECTSINGLESHOOTING(LAGRANGE,MAYER,F,EQPC,INPC,EQTC,INTC,X0,TF,N,M,'given',MET)
%   solves a continuous time optimal control problem based on given Bolza's
...
FUN = @(w) bolza(w,lagrange,mayer,f,x0,tf,N,intMethod,varargin);
A = [];
B = [];
Aeq = [];
Beq = [];
...
function b = bolza(w,lagrange,mayer,f,x0,tf,N,intMethod,varargin)
%BOLZA - calculates the total cost based on given Bolza's formulation 
%   B = BOLZA(W,L,M,F,X0,TF,N,INT) calculates the total cost based on given
%   guess W, Lagrange's term L, Mayer's term M, a dynamic system F, an 
...
% Forward Simulate to obtain state trajectroy based on guessed
% control input vector

[XSim,~] = forwardSimulation(f,x0,UGuess,tf,N,intMethod,varargin{:});

% Numerically evaluate Bolza's formulation using the guess

[b,~,~] = bolzaCost(lagrange,mayer,XSim,UGuess,tf,intMethod,varargin{:});
...
%% Forward Simulate to obtain state trajectroy based on guessed control input vector
...
dimEqPathCon = size(eqPathCon(XSim(:,1),UGuess(:,1)),1);
dimEqTerCon = size(eqTerCon(XSim(:,end),tf),1);
eqCon = zeros(dimEqPathCon*(N+1)+dimEqTerCon,1);

% Allocate space for inequality constraint

dimInPathCon = size(inPathCon(XSim(:,1),UGuess(:,1)),1);
dimInTerCon = size(inTerCon(XSim(:,end),tf),1);
inCon = zeros(dimInPathCon*(N+1)+dimInTerCon+dimTF,1);

% Path Constraint
for i = 1:N,
    eqCon(dimEqPathCon*(i-1)+1:dimEqPathCon*i,1) = eqPathCon(XSim(:,i),UGuess(:,i));
    inCon(dimInPathCon*(i-1)+1:dimInPathCon*i,1) = inPathCon(XSim(:,i),UGuess(:,i));
end
eqCon(dimEqPathCon*N+1:dimEqPathCon*(N+1),1) = eqPathCon(XSim(:,N+1),UGuess(:,N));

% Terminal Constraint

eqCon(dimEqPathCon*(N+1)+1:dimEqPathCon*(N+1)+dimEqTerCon,1) = eqTerCon(XSim(:,end),tf);
inCon(dimInPathCon*N+1:dimInPathCon*N+dimInTerCon,1) = inTerCon(XSim(:,end),tf);
